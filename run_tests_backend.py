...
# -*- coding: utf-8 -*-
"""
Script para executar todos os testes do compilador completo (backend)
Testa a compilação completa incluindo geração de código LLVM
"""
...
def run_test(file_path, expected_success=True):
    ...
    try:
        # Executa o compilador completo com geração de IR
        result = subprocess.run(
            [sys.executable, "main.py", str(file_path), "--run"],
            capture_output=True,
            text=True,
            encoding='utf-8',
            errors='replace'
...
def main():
    ...
    print("="*60)
    print("EXECUTANDO TODOS OS TESTES DO COMPILADOR (BACKEND)")
    print("="*60)
    
    # Testes válidos (devem passar)
    valid_tests = sorted((base_path / "testes" / "valid").glob("*.ts"))
    
    # Testes inválidos (devem falhar) - pega apenas o primeiro
    invalid_tests = sorted((base_path / "testes" / "invalid").glob("*.ts"))[:1]
